...
- [x] **ATTEST-01**: Users can list open execution follow-ups and attach resulting run and
  comparison artifacts to a specific execution receipt from the CLI.
- [x] **ATTEST-02**: Users can finalize an execution outcome attestation with a persisted closure
  state, linked evidence, and operator notes without mutating family policy implicitly.

### Measured Outcome Verdicts

- [x] **VERDICT-01**: The system computes a deterministic outcome verdict (`improved`,
  `regressed`, `inconclusive`, or `no_signal`) from linked follow-up comparison evidence.
- [x] **VERDICT-02**: Every attested outcome preserves the compared signals, delta summary, and
  rationale used to reach the verdict.

### Policy Feedback

- [x] **FEEDBACK-01**: Attested outcomes feed back into dataset-family history and portfolio
  priority context so operators can see whether prior actions helped.
- [x] **FEEDBACK-02**: Future plan review and execution surfaces expose relevant prior attested
  outcomes for the same family or action type before new changes are applied.

### Debugger Outcome Closure

- [x] **UI-01**: Existing debugger routes surface open follow-ups, attested verdicts, and
  action-effect timelines without introducing a separate outcome dashboard.
- [x] **UI-02**: Users can move from an execution receipt to its linked follow-up evidence,
...
|-------------|-------|--------|
| ATTEST-01 | Phase 121 | Complete |
| ATTEST-02 | Phase 121 | Complete |
| VERDICT-01 | Phase 122 | Complete |
| VERDICT-02 | Phase 122 | Complete |
| FEEDBACK-01 | Phase 123 | Complete |
| FEEDBACK-02 | Phase 123 | Complete |
| UI-01 | Phase 123 | Complete |
| UI-02 | Phase 123 | Complete |
| FLOW-01 | Phase 124 | Complete |
