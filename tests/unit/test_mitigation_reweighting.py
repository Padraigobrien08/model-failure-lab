...
def test_build_group_weight_table_uses_train_only_group_counts():
    ...
    weights = build_group_weight_table(records)
    weight_lookup = {
        row["group_id"]: row["sample_weight"]
        for row in weights.to_dict(orient="records")
    }

    assert list(weights["group_id"]) == ["group_a", "group_b"]
    assert weight_lookup["group_b"] > weight_lookup["group_a"]
    assert pytest.approx(
        sum(weight_lookup[row["group_id"]] for row in records) / len(records),
        rel=1e-6,
    ) == 1.0


def test_train_distilbert_reweighting_writes_group_weights_and_predictions(
    temp_artifact_root,
):
    config = load_experiment_config("configs/experiments/civilcomments_distilbert_reweighting.yaml")
    config["run_id"] = "distilbert_reweighting_test"
    config["parent_run_id"] = "distilbert_parent"
    config["parent_model_name"] = "distilbert"
    config["mitigation_method"] = "reweighting"
    ...
    artifacts = train_distilbert_reweighting(
        config,
        run_dir,
    ...
    assert artifacts.group_weights_path.exists()
